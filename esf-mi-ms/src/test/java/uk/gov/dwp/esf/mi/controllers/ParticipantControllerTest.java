...
public class ParticipantControllerTest {
	...
	/*
	 * Test getAllParticipants controller rest method This method
	 * returns with basic participant information 
	 * A self link for each participant,
	...
	public void testGetAllParticipants() throws Exception {
		...
		final Participant participant = new Participant.ParticipantBuilder("56ebb854bf9a401670d0eadc", 1111111,
				...
				.recordState(RecordState.CREATED).build();
		...
		when(participantService.getAllParticipants(Mockito.anyObject(),Mockito.anyString())).thenReturn(participantResources);
		...
		mockMvc.perform(get(uri).accept(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.participants.[0].participant.recordState").value(equalTo("CREATED")))
				.andExpect(jsonPath("$.participants.[0].links.[0].href")
						.value(equalTo("/participants/56ebb854bf9a401670d0eadc")));


		when(participantService.getAllParticipants( Mockito.anyObject(),
	...
	/*
	 * Test getParticipant controller rest method
	 * This method returns all participant details for the specified participantId
	 * A self link for the participant object is created.
	 */
	...
	public void testGetParticipant() throws Exception {
		...
		when(participantService.getParticipant(Mockito.anyString(),
				Mockito.anyObject(), Mockito.anyString())).thenReturn(participantList);

		String uri = "/participants/{participantId}";

		mockMvc.perform(get(uri, "12345678").accept(MediaType.APPLICATION_JSON_UTF8))
				...
				.andExpect(jsonPath("$.[0].participant.recordState").value(equalTo("CREATED")))
				.andExpect(jsonPath("$.[0].links.[0].href").value(equalTo("/participants/12345678")));


		when(participantService.getParticipant( Mockito.anyString(),
				Mockito.anyObject(), Mockito.anyString())).thenReturn(null);
		mockMvc.perform(get(uri, "5555555").accept(MediaType.APPLICATION_JSON_UTF8))
	...
	public void testGetFilteredParticipants() throws Exception {
		...
		final Participant participant1 = new Participant.ParticipantBuilder("12345677", 1111111, "SL123457B",
				dateFormat.parse(dateFormat.format(new Date())),4001115).creationDate(dateFormat.parse(dateFormat.format(new Date())))
						.updatedDate(dateFormat.parse(dateFormat.format(new Date()))).recordState(RecordState.CREATED).build();
		...
		String uri = "/participants/search/{filter}";

		mockMvc.perform(get(uri, filter).accept(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				...
				.andExpect(jsonPath("$.participants.[0].participant.recordState").value(equalTo("CREATED")))
				.andExpect(jsonPath("$.participants.[0].links.[0].href")
						.value(equalTo("/participants/12345677")))
				.andExpect(jsonPath("$.participants.[1].links.[0].href")
						.value(equalTo("/participants/12345678")))
				.andExpect(jsonPath("$.participants.[2].links.[0].href")
						.value(equalTo("/participants/12345679")));


		when(participantService.getFilteredParticipants(Mockito.anyString(), Mockito.anyObject(), Mockito.anyString()))
				.thenReturn(null);
		mockMvc.perform(get(uri, filter).accept(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isNotFound()).andDo(print());
	 ...
	 * participant object in the back end and returns the stored Participant object.
	 * A self link for the participant object is created.
	 */
	@Test
	public void testCreateParticipant() throws Exception {
		String REQUEST_CONTENT = "{\"participantId\":\"56b4694d7eb68a5fff76290c\",\"providerId\":1111111,\"nino\":\"SJ196058B\",\"dob\":\"1982-12-16\",\"contractId\":4001115,\"creationDate\":\"2015-01-11\",\"updatedDate\":\"2015-01-11\",\"recordState\":\"CREATED\"}";
		final List<Participant> participantsList = new ArrayList<>();
		final Participant participant = new Participant.ParticipantBuilder(null, 1111111, "SJ196058B",dateFormat.parse("1982-12-16"),4001115)
						.creationDate(dateFormat.parse("2015-01-11")).updatedDate(dateFormat.parse("2015-01-11")).recordState(RecordState.CREATED)
						.build();
	...
	public void testUpdateParticipant() throws Exception {
		...
		mockMvc.perform(
				put(uri,"56b4694d7eb68a5fff76290c").contentType(MediaType.APPLICATION_JSON_UTF8).accept(MediaType.APPLICATION_JSON_UTF8)
	 ...
	 * This method deletes a participant object in the back end for the given participant Id.
	 */
	@Test
	public void testDeleteParticipant() throws Exception {
		String uri = "/participants/{participantId}";
		mockMvc.perform(delete(uri, "56b4694d7eb68a5fff76290c")).andExpect(status().isOk()).andDo(print());
